...
export class CartPageComponent implements OnInit {
  ...
  price:environment.numberZero,
  discount:environment.numberZero,
  tax:environment.numberZero,
  delivery:environment.numberZero,
  total:environment.numberZero
  }
  ...
  constructor(private product:ProductService,private router:Router,private http: HttpClient,private user:UserService) {
    this.name=user.name;
   }
...
productQuantity:number=1;
 handleQuantity(value:string){
   if(this.productQuantity<environment.numbertwenty && value==='plus'){
    this.productQuantity+=environment.numberOne;
   }else if(this.productQuantity>environment.numberOne && value==='min'){
    this.productQuantity-=environment.numberOne;
   }
   }
...
loadDetails(){
  this.product.currentCart().subscribe((result) => {
   ...
   let discount = environment.numberZero;
   let tax = environment.numberZero;
   this.priceSummary.total=((price+environment.numberPercent)+(+tax)-(+discount));

  localStorage.setItem("total amount" ,this.priceSummary.total.toString());
